-- SSC With Jagrat — UPDATE v26 (Advanced Admin/CMS + Bulk Question Management)
-- SAFE / ADDITIVE: existing questions, attempts, subscriptions, progress and files are preserved.
-- Run AFTER database_update_v25_advanced_analytics.sql.

alter table public.quiz_questions add column if not exists archived_at timestamptz;
create index if not exists quiz_questions_archived_idx on public.quiz_questions(archived_at);

-- Keep archived questions out of the public question-bank/practice flows while retaining the rows for recovery.
...
begin
 ...
 where q.archived_at is null and c.published=true and (coalesce((regexp_match(trim(c.class_no),'[0-9]+'))[1]::int=1,false) or v_has_sub)
...
-- Rebuild practice submission so archived questions cannot be selected after an admin archive.
...
begin
 ...
 if exists(select 1 from public.quiz_questions q join public.classes c on c.id=q.class_id where q.id=any(p_question_ids) and q.archived_at is not null) then raise exception 'One or more selected questions are no longer available.' using errcode='22023'; end if;
 ...
 into v_total,v_answered,v_correct,v_wrong,v_max,v_score from public.quiz_questions q join public.classes c on c.id=q.class_id where q.id=any(p_question_ids) and q.archived_at is null and c.published=true;
...
comment on column public.quiz_questions.archived_at is 'Non-destructive admin archive timestamp; archived questions are retained but excluded from student practice flows.';


-- Admin-only consolidated question-bank feed. It intentionally returns the answer key because
-- this function is restricted to the existing site administrator account.
create or replace function public.admin_question_bank_v26()
returns setof jsonb
language plpgsql security definer set search_path=public,pg_temp as $$
begin
 if lower(coalesce(auth.jwt()->>'email','')) <> 'jalajsinghal04@gmail.com' then
   raise exception 'Admin access required.' using errcode='42501';
 end if;
 return query
 select jsonb_build_object('id',q.id,'class_id',q.class_id,'class_title',c.title,'class_no',c.class_no,
   'subject_name',s.name,'question',q.question,'option_a',q.option_a,'option_b',q.option_b,'option_c',q.option_c,'option_d',q.option_d,
   'correct_option',q.correct_option,'explanation',q.explanation,'topic',q.topic,'difficulty',coalesce(q.difficulty,'medium'),
   'is_pyq',q.is_pyq,'pyq_year',q.pyq_year,'pyq_tier',q.pyq_tier,'marks',q.marks,'negative_marks',q.negative_marks,'archived_at',q.archived_at,
   'sort_order',q.sort_order,'created_at',q.created_at)
 from public.quiz_questions q join public.classes c on c.id=q.class_id join public.subjects s on s.id=c.subject_id
 order by s.sort_order,c.created_at,q.sort_order,q.created_at;
end;$$;
revoke all on function public.admin_question_bank_v26() from public;
grant execute on function public.admin_question_bank_v26() to authenticated;
